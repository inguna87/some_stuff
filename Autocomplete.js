// ==UserScript==
// @name           Autocomplete
// @description    Autocomplete similar to the one in FF
// @include        *
// ==/UserScript==
...
(function(opera) {
		...
		if (data) {
			...
			for (var i = 0; i < splitedData.length; i++) {
				if (splitedData[i].match(new RegExp(e.target.value, "i"))) {
					results.push('<span style="font-weight: bold;" onmouseover="this.parentNode.className=\'selected\'">' + splitedData[i].replace(new RegExp('(' + e.target.value + ')', "i"), '<span style="font-weight: normal;" onmouseover="this.parentNode.className=\'selected\'">$1</span>') + '</strong>');
				}
			}
			if (results.length) {
	...
	window.addEventListener('submit', function(e) {
		...
		for (var index = 0; index < inputArray.length; index++) {
			if ((inputArray[index].type == 'text' || inputArray[index].type == 'url' || inputArray[index].type == 'email' || inputArray[index].type == 'search' || inputArray[index].type == 'tel' || inputArray[index].type == 'number') && inputArray[index].value > '') {
	...
	window.addEventListener('load', function(e) {
		...
		style.innerHTML = '#autocomplete {font-family: arial,sans-serif; font-size: 13px; font-weight: normal; line-height: normal; color: #000; text-align: left; position: absolute; overflow: auto; max-height: 90px; background-color: #fff; border: solid 1px; z-index: 100;} div#autocomplete div.selected {background-color: #000099; color: #fff;} #helperDiv {position: absolute; visibility: hidden; width: auto;}';
		document.getElementsByTagName('head')[0].appendChild(style);
		var inputs = document.getElementsByTagName("input");
		for (var index = 0; index < inputs.length; index++) {
			if (inputs[index].type!= 'text' && inputs[index].type!= 'url' && inputs[index].type!= 'email' && inputs[index].type!= 'search' && inputs[index].type!= 'tel' && inputs[index].type!= 'number') {
				continue;
			}
			inputs[index].addEventListener('dblclick', function(e) {
				getData(e);
			}, true);
